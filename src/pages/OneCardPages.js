...
function get_one_card_page_routes(state){
    if ('user_data' in state) {
        ...
        console.log('get_one_card_page_routes');
        console.log(state);
        state.user_data.users.forEach(user => {
            USER = user;
            routes.push(<Route exact path={'/cards/' + user.id.toString()}
                               component={get_one_card_page_render(user.username,
                                   PatientCard(user.username, InputForm, state.user_data.events[user.id]))}/>);
